...
The main RyuGraphWidget class is defined in this module.
...
RYUGRAPH_LABEL_KEYS = ['name', 'title', 'text', 'description', 'caption', 'label']

class RyuGraphWidget:
    """
    A yFiles Graphs for Jupyter widget that is tailored to visualize Cypher queries resolved against a RyuGraph database.
    """
    ...
    def __init__(self, connection: Optional[Any] = None, widget_layout: Optional[Any] = None,
        ...
        """
        Initializes a new instance of the RyuGraphWidget class.

        Args:
            connection (Optional[ryugraph connection type]): The RyuGraph connection to resolve the Cypher queries.
        ...
        # Holds the most recent RyuGraph query result from show_cypher(); None until first call
    ...
    def set_connection(self, connection: Any) -> None:
        """
        The RyuGraph connection that is used to resolve the Cypher queries. A new session is created when set.

        Args:
            connection: The RyuGraph connection to resolve the Cypher queries.
    ...
    def get_connection(self) -> Any:
        """
        Gets the configured RyuGraph connection.
    ...
    def connection(self) -> Any:
        """The configured RyuGraph connection.
    ...
    def connection(self, value: Any) -> None:
        """Sets the RyuGraph connection used to resolve Cypher queries."""
    ...
    def query_result(self) -> Optional[Any]:
        """
        The most recent RyuGraph query result produced by show_cypher(), or None if show_cypher() has not been called yet.
        """
        return self._query_result

    def _parse_query_result(self, query_result: Any) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        """
        Parse a RyuGraph graph database query result DataFrame into nodes and relationships.

        Args:
            query_result: QueryResult generator from RyuGraph that contains a list of query results
        ...
        if self._connection is None:
            raise ValueError("No database connection provided. Initialize widget with a valid RyuGraph connection.")
        ...
        for node in node_map.values():
            ...
            while node_tbl_info.has_next():  # type: ignore
                ...
                if prop[-1] is True:
                    # RyuGraph enforces that every node table MUST have one and only one primary key, so
    ...
    def show_cypher(self, cypher: str, layout: Optional[str] = None, **kwargs: Dict[str, Any]) -> None:
        ...
        if self._connection is not None:
            ...
            try:
                ...
                logging.warning(f"RyuGraphWidget: Cannot display interactive graph visualization. Ensure that the query returns nodes and relationships.\nRoot Cause: {e}")
                return
    ...
    def __get_item_text(element: Dict) -> Union[str, None]:
        ...
        for key in RYUGRAPH_LABEL_KEYS:
    ...
    def __configuration_mapper_factory(binding_key: str, configurations: Dict[str, Dict[str, str]],
        ...
        def mapping(index: int, item: Dict) -> Union[Dict, str]:
            label = item["properties"]["label"]  # yjg stores the ryugraph node/relationship type in properties["label"]
            ...
            if binding_key == "label":
                return RyuGraphWidget.__get_item_text(item)
            else:
    ...
    def __apply_heat_mapping(self, configuration, widget: GraphWidget) -> None:
        setattr(widget, "_heat_mapping",
                RyuGraphWidget.__configuration_mapper_factory('heat', configuration,
    ...
    def __apply_node_mappings(self, widget: GraphWidget) -> None:
        for key in POSSIBLE_NODE_BINDINGS:
            ...
            setattr(widget, f"_node_{key}_mapping",
                    RyuGraphWidget.__configuration_mapper_factory(key, self._node_configurations, default_mapping))
        # manually set parent configuration
        setattr(widget, f"_node_parent_mapping",
                RyuGraphWidget.__configuration_mapper_factory('parent_configuration',
    ...
    def __apply_edge_mappings(self, widget: GraphWidget) -> None:
        for key in POSSIBLE_EDGE_BINDINGS:
            ...
            setattr(widget, f"_edge_{key}_mapping",
                    RyuGraphWidget.__configuration_mapper_factory(key, self._edge_configurations, default_mapping))
    ...
    def del_node_configuration(self, label: Union[str, list[str]]) -> None:
        ...
        if isinstance(label, list):
            for l in label:
                RyuGraphWidget.__safe_delete_configuration(l, self._node_configurations)
        else:
            RyuGraphWidget.__safe_delete_configuration(label, self._node_configurations)
    ...
    def del_relationship_configuration(self, type: Union[str, list[str]]) -> None:
        ...
        if isinstance(type, list):
            for t in type:
                RyuGraphWidget.__safe_delete_configuration(t, self._edge_configurations)
        else:
            RyuGraphWidget.__safe_delete_configuration(type, self._edge_configurations)
